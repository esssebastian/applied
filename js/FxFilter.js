class FxFilter {
  ...
  static addFxContainer(element, filterValue, parsedFilter) {
    ...
    if (backdropFilter.trim()) {
      // Create the structure. Tag our injected SVG so cleanup never removes user SVGs.
      element.innerHTML += `
  ...
  static removeFxContainer(element) {
    // Only remove our own artifacts, never user content SVGs
    element
      .querySelectorAll(
        '.fx-container, svg.fx-svg-defs, svg[data-fx-svg="true"]'
      )
      ...
      for (let y = 0; y < topOffset; y++) {
        for (let x = 0; x < maxDimension; x++) {
          ...
          data[pixelIndex + 2] = Math.max(
            ...
            Math.min(
              255,
              Math.round(127 + 127 * vyValue * Math.pow(gradientSegment, 1))
            )
          );
        }
      }
      ...
      for (let y = 0; y < maxDimension; y++) {
        for (let x = 0; x < leftOffset; x++) {
          ...
          data[pixelIndex] = Math.max(
            ...
            Math.min(
              255,
              Math.round(127 + 127 * vxValue * Math.pow(gradientSegment, 1))
            )
          );
        }
      }
